...
fig_vis, axes_vis = plt.subplots(2, 5, figsize=(12, 6))
...
# %%
def create_token_training_data(quantized_dataset, context_length, start_token_int, num_pixel_values, max_samples=1000000, max_images_to_process=None):
    """
...
def create_randomized_token_training_data_with_pos(quantized_dataset, context_length, start_token_int, num_pixel_values, 
                                       img_total_pixels, max_samples=100000, max_images_to_process=None):
    """
...
class CategoricalPixelPredictor(nn.Module):
    def __init__(self, num_pixel_values, token_embedding_dim, context_length, 
                 hidden_size, img_size, pos_embedding_dim, start_token_integer_value, dropout_rate=0.25):
        """
        Args:
            ...
            start_token_integer_value (int): The integer value used for the start token.
                                            Used to determine total vocabulary size for token embedding.
        ...
        # Vocabulary size for the token embedding layer:
        # Needs to include K pixel tokens + 1 start token.
        # If pixel tokens are 0..K-1 and start_token_integer_value is K,
        # then max token value is K. Embedding needs K+1 entries.
        self.token_vocab_size = max(num_pixel_values - 1, start_token_integer_value) + 1
        
        self.token_embedding = nn.Embedding(num_embeddings=self.token_vocab_size, 
        ...
        # Input to fc1: (flattened context token embeddings) + (row_pos_embed) + (col_pos_embed)
        context_features_dim = context_length * token_embedding_dim
        positional_features_dim = 2 * pos_embedding_dim
        self.mlp_input_dim = context_features_dim + positional_features_dim
    ...
    def forward(self, x_context_tokens, pixel_positions_flat, training=True):
        ...
        Args:
            x_context_tokens (Tensor): Batch of context windows. Shape: [batch_size, CONTEXT_LENGTH]. Integer tokens.
            pixel_positions_flat (Tensor): Absolute flat positions (0 to N_PIXELS-1) of target pixels. Shape: [batch_size].
            training (bool): Whether in training mode.
        """
        batch_size = x_context_tokens.shape[0]
        
        # 1. Get learned embeddings for context tokens
        # x_context_tokens contains integers from 0 to token_vocab_size-1
        embedded_context = self.token_embedding(x_context_tokens)  # Shape: [batch_size, CONTEXT_LENGTH, token_embedding_dim]
        flattened_embedded_context = embedded_context.view(batch_size, -1)
        
        # 2. Get positional embeddings for the target pixel
        ...
        h = F.relu(self.fc2(h))
        if training:
            h = self.dropout(h) # Consider if this second dropout is beneficial or should be tuned
        output_logits = self.fc_out(h)
...
model_cat_predictor = CategoricalPixelPredictor(
    ...
    img_size=IMG_SIZE,
    pos_embedding_dim=POS_EMBEDDING_DIM,
    start_token_integer_value=START_TOKEN_VALUE_INT, # To correctly size token_embedding layer
)

print("--- Model V3: CategoricalPixelPredictor (Token Embeddings + Positional Embeddings) ---")
print(f"  Number of actual pixel intensity tokens (K): {NUM_QUANTIZATION_BINS}")
print(f"  Token Embedding Vocabulary Size (max_token_val + 1): {model_cat_predictor.token_vocab_size}")
print(f"  Token Embedding Dimension: {TOKEN_EMBEDDING_DIM}")
print(f"  Positional Embedding Dimension (per row/col): {POS_EMBEDDING_DIM}")
print(f"  Context Length: {CONTEXT_LENGTH}")
print(f"  Image Size: {IMG_SIZE}x{IMG_SIZE}")
print(f"  Hidden Size: {HIDDEN_SIZE}")
print(f"  Total MLP Input Dimension: {model_cat_predictor.mlp_input_dim}")
print(f"  Model V3 parameters: {sum(p.numel() for p in model_cat_predictor.parameters()):,}")
...
def generate_with_token_probabilities(model, context_length, start_token_int, num_pixel_values,
    ...
    with torch.no_grad():
        for i in range(total_pixels): # Iterate through each pixel position
            current_pixel_position = torch.tensor([i], dtype=torch.long, device=device)
            output_logits = model(current_context_tokens, pixel_positions_flat=current_pixel_position)
            probabilities = F.softmax(output_logits / temperature, dim=-1).squeeze()
...
# %% [markdown]
# ### Training Data for Model V3
#
# Fortunately, the training data required for Model V3 (`CategoricalPixelPredictor`) is identical in format to what we prepared for Model V2:
# 1.  *Context Windows (`train_contexts`):* Sequences of integer tokens representing the history. Model V3 will now pass these integer tokens through its `nn.Embedding` layer.
# 2.  *Target Tokens (`train_targets`):* The integer token of the actual next pixel.
# 3.  *Target Pixel Positions (`train_positions`):* The absolute flat position of the target pixel, used for the positional embeddings.
#
# So, we can directly reuse the `train_contexts`, `train_targets`, and `train_positions` tensors we generated previously.

# %%
print("--- Reusing Data for Model V3 (CategoricalPixelPredictor) ---")
print(f"  train_contexts shape: {train_contexts.shape}")
print(f"  train_targets shape: {train_targets.shape}")
print(f"  train_positions shape: {train_positions.shape}")
...
#
# The training setup (loss function, optimizer) will be the same as for Model V2. We expect that the richer input representation provided by token embeddings will allow the model to learn more effectively and hopefully generate higher-quality images.
...
criterion = nn.CrossEntropyLoss()
optimizer = optim.AdamW(model_cat_predictor.parameters(), lr=LEARNING_RATE, weight_decay=1e-5)

n_samples = len(train_contexts)
if n_samples == 0:
    print("No training samples available for Model V3. Skipping training.")
else:
    print(f"\nTraining Model V3 (CategoricalPixelPredictor) on {n_samples:,} samples for {EPOCHS} epochs.")

    # --- Training Loop for Model V3 ---
    epoch_pbar = auto_tqdm(range(EPOCHS), desc="Model V3 Training Epochs", position=0, leave=True)
    for epoch in epoch_pbar:
        ...
        for start_idx in batch_pbar:
            ...
            batch_context_tokens = train_contexts[batch_indices]
            batch_target_tokens = train_targets[batch_indices]
            batch_pixel_positions = train_positions[batch_indices]
...
#
# With our `CategoricalPixelPredictor` (Model V3) trained, it's time for the moment of truth! This model uses learned embeddings for pixel intensity tokens and positional encodings, representing our last approach in this tutorial.
#
# The generation process will be identical to that of Model V2, as Model V3 expects the same inputs during inference:
# 1.  An initial context window of `START_TOKEN_VALUE_INT`.
# 2.  At each step `i` (from 0 to `N_PIXELS-1`):
#     a.  The current context window of (integer) tokens.
#     b.  The current pixel's absolute flat position (`i`).
# 3.  The model outputs logits, we apply temperature, sample, and update the context.
#
# We are particularly interested to see if the combination of learned token relationships (via embeddings) and spatial awareness (via positional encodings) leads to a noticeable improvement in the quality and recognizability of the generated MNIST digits compared to Models V1 and V2.

# %%
# Ensure dequantize_tensor, IMG_SIZE, NUM_QUANTIZATION_BINS, CONTEXT_LENGTH, 
# START_TOKEN_VALUE_INT, N_PIXELS, device are all accessible.

def generate_image_v3(model, context_length, start_token_int, num_pixel_values_k, 
                      img_size, total_num_pixels, current_device, temperature=1.0):
    """
    Generates a single image using Model V3 (CategoricalPixelPredictor).
    """
    model.eval()
    model.to(current_device)
    ...
    pixel_gen_pbar = auto_tqdm(range(total_num_pixels), desc="Generating V3 Image Pixels", leave=False, position=0, disable=True) # Disable for multi-image plot

    with torch.no_grad():
        for i in pixel_gen_pbar: # i is the current_flat_pixel_position
            current_flat_pixel_position_tensor = torch.tensor([i], dtype=torch.long, device=current_device)
            
            # Model V3 forward pass
            output_logits = model(
                current_context_tokens_tensor, 
                pixel_positions_flat=current_flat_pixel_position_tensor,
                training=False 
            )
...
if n_samples > 0: # Only generate if Model V3 was trained
    print("\n--- Generating Images from Model V3 (Token Embeddings + Position) ---")
    n_images_to_generate = 25 # 5x5 grid
    fig_gen, axes_gen = plt.subplots(5, 5, figsize=(8, 8))
    fig_gen.suptitle(f"Model V3 Generated Digits ({NUM_QUANTIZATION_BINS} Bins, TokenEmb, PosEmb)", fontsize=14)

    for i in range(n_images_to_generate):
        row, col = i // 5, i % 5
        ax = axes_gen[row, col]
        
        dequantized_img = generate_image_v3(
            model_cat_predictor, 
            CONTEXT_LENGTH, 
            START_TOKEN_VALUE_INT, 
            NUM_QUANTIZATION_BINS, 
            ...
            N_PIXELS,
            device, 
            temperature=1.0 # Experiment with temperature (e.g., 0.8, 0.9 for sharper)
        )
        ax.imshow(dequantized_img, cmap='gray', vmin=0, vmax=1)
        ax.axis('off')
